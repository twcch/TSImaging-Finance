...
class Model(nn.Module):
    """
    GAF-CNN + RevIN（對應 GAF-CNN 的 RevIN 版本）。

    結構與 models/GAF-CNN.py 完全相同（GASF 角度關係影像化 + 簡單 2D-CNN），
    差異在於預測（forecast）路徑加入可學習仿射的 RevIN（Reversible Instance
    Normalization）：在影像化之前對輸入做 'norm'、在輸出端做 'denorm'，使預測
    值還原回原始尺度，緩解金融序列的非平穩性。

    分類（classification）路徑維持與 GAF-CNN 基準一致：GASF 內部已對每個視窗做
    min-max 正規化，且分類輸出為 logits 無需反正規化，故不額外加入 RevIN。
